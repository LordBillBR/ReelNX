...
#define DEFAULT_NRO "sdmc:/switch/ReelNX.nro"
#define APP_STORE_NRO "sdmc:/switch/ReelNX/ReelNX.nro"
...
static Result readSetting(const char* key, void* buf, size_t size) {
    ...
    if (R_SUCCEEDED(rc) && actual_size != size) rc = MAKERESULT(Module_Libnx, LibnxError_BadInput);
    if (R_SUCCEEDED(rc)) rc = setsysGetSettingsItemValue(section_name, key, buf, size, &actual_size);
    if (R_SUCCEEDED(rc) && actual_size != size) rc = MAKERESULT(Module_Libnx, LibnxError_BadInput);
...
void __appInit(void) {
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc)) hosversionSet(ams_flag | MAKEHOSVERSION(fw.major, fw.minor, fw.micro));
...
static void setupHbHeap(void) {
    ...
    if (R_FAILED(rc) || addr == NULL) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 9));
...
static void procHandleReceiveThread(void* arg) {
    ...
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 15));

    HipcParsedRequest r = hipcParseRequest(base);
    if (r.meta.num_copy_handles != 1) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 17));
...
static void getIsAutomaticGameplayRecording(void) {
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc) && data.nacp.video_capture == 2) g_isAutomaticGameplayRecording = 1;
    }
}
...
static void getOwnProcessHandle(void) {
    ...
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 12));

    Thread t;
    rc = threadCreate(&t, &procHandleReceiveThread, (void*)(uintptr_t)server_handle, NULL, 0x1000, 0x20, 0);
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 10));

    rc = threadStart(&t);
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 13));
...
void loadNro(void) {
    ...
    if (read(fd, start, sizeof(*start)) != sizeof(*start)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 4));

    if (read(fd, header, sizeof(*header)) != sizeof(*header)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 4));

    if (header->magic != NROHEADER_MAGIC) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 5));

    size_t rest_size = header->size - (sizeof(NroStart) + sizeof(NroHeader));
    if (read(fd, rest, rest_size) != rest_size) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 7));
    ...
    // .data + .bss
    rc = svcSetProcessMemoryPermission(g_procHandle, (u64)map_addr + header->segments[2].file_off, rw_size, Perm_Rw);
...
#define M EntryFlag_IsMandatory
    ...
    static ConfigEntry entries[] = {{EntryType_MainThreadHandle, 0, {0, 0}}, {EntryType_ProcessHandle, 0, {0, 0}},
        {EntryType_AppletType, 0, {AppletType_LibraryApplet, 0}}, {EntryType_OverrideHeap, M, {0, 0}},
        {EntryType_Argv, 0, {0, 0}}, {EntryType_NextLoadPath, 0, {0, 0}}, {EntryType_LastLoadResult, 0, {0, 0}},
        {EntryType_SyscallAvailableHint, 0, {UINT64_MAX, UINT64_MAX}},
        {EntryType_SyscallAvailableHint2, 0, {UINT64_MAX, 0}}, {EntryType_RandomSeed, 0, {0, 0}},
        {EntryType_UserIdStorage, 0, {(u64)(uintptr_t)&g_userIdStorage, 0}}, {EntryType_HosVersion, 0, {0, 0}},
