...
void BaseCardCell::setPrimaryActionLabel(const std::string& label) {
    this->primaryActionLabel = label;
    if (this->actionLabel) this->actionLabel->setText(label);
    if (this->actionBox && (label.empty() || !this->isFocused())) this->actionBox->setVisibility(brls::Visibility::GONE);
}

void BaseCardCell::onFocusGained() {
    RecyclingGridItem::onFocusGained();
    if (this->actionBox && !this->primaryActionLabel.empty()) {
        this->actionLabel->setText(this->primaryActionLabel);
        this->actionBox->setVisibility(brls::Visibility::VISIBLE);
    }
}

void BaseCardCell::onFocusLost() {
    if (this->actionBox) this->actionBox->setVisibility(brls::Visibility::GONE);
