...
void openSearch() {
    brls::Application::getImeManager()->openForText(
        [](const std::string& text) {
            if (!text.empty()) brls::Application::pushActivity(new brls::Activity(new StremioSearch(text)));
        },
...
public:
    ...
    RecyclingGridItem* cellForRow(RecyclingView* recycler, size_t index) override {
        ...
        cell->setPrimaryActionLabel("A Details");
        if (item.year.empty()) {
...
public:
    ...
    RecyclingGridItem* cellForRow(RecyclingView* recycler, size_t index) override {
        ...
        if (index == this->list.size()) {  // the See-all card
            ...
            cell->setPrimaryActionLabel("A Open");
    ...
    void onItemSelected(brls::Box* recycler, size_t index) override {
        if (index == this->list.size()) {
            brls::Application::pushActivity(new brls::Activity(new StremioLibrary()), brls::TransitionAnimation::NONE);
            return;
        }
...
public:
    ...
    RecyclingGridItem* cellForRow(RecyclingView* recycler, size_t index) override {
        ...
        if (index == this->list.size()) {  // the More card
            ...
            cell->setPrimaryActionLabel("A Browse");
    ...
    void onItemSelected(brls::Box* recycler, size_t index) override {
        if (index == this->list.size()) {
            brls::Application::pushActivity(new brls::Activity(new StremioAnime()), brls::TransitionAnimation::NONE);
            return;
        }
        brls::Application::pushActivity(
            new brls::Activity(new StremioDetail(this->list.at(index))), brls::TransitionAnimation::NONE);
...
public:
    ...
    RecyclingGridItem* cellForRow(RecyclingView* recycler, size_t index) override {
        ...
        cell->setPrimaryActionLabel("A Resume");
        cell->labelExt->setVisibility(brls::Visibility::GONE);
        cell->labelRating->setVisibility(brls::Visibility::INVISIBLE);
        cell->badgeFavorite->setVisibility(brls::Visibility::INVISIBLE);
        cell->badgeTopRight->setVisibility(brls::Visibility::GONE);
...
void StremioHome::updateActionBar() {
    if (!this->actionBar) return;

    brls::View* focus = brls::Application::getCurrentFocus();
    bool continueContext = false;
    bool rowContext = false;
    for (brls::View* f = focus; f != nullptr; f = f->getParent()) {
        if (f == this->continueRec) {
            continueContext = true;
            break;
        }
        if (dynamic_cast<HRecyclerFrame*>(f)) rowContext = true;
    }

    bool canToggleList = false;
    std::string primary = "Details";
    if (auto* cell = dynamic_cast<FavCardCell*>(focus)) {
        canToggleList = (bool)cell->onToggleFav;
        const std::string& inlineLabel = cell->getPrimaryActionLabel();
        if (inlineLabel.rfind("A ", 0) == 0) primary = inlineLabel.substr(2);
    }

    std::string context = continueContext ? "continue" : rowContext ? (canToggleList ? "card-list" : "card") : "chrome";
    context += ":" + primary;
    if (context == this->actionBarContext && this->actionBar->getChildren().size() > 0) return;
    this->actionBarContext = context;

    this->actionBarContinueContext = continueContext;
    if (continueContext) {
        this->actionBar->setActions({
            {brls::BUTTON_A, "Resume"},
...
// Top bar: small ReelNX mark on the left, a round search button on the
...
void StremioHome::addTopBar() {
    ...
    auto* logo = new SVGImage();
    logo->setDimensions(42, 42);
    logo->setImageFromSVGRes("icon/reelnx-mark.svg");
...
void StremioHome::addRow(const std::string& title, const std::string& url) {
    ...
    stremio::getJSON<stremio::MetaList>(
        ...
        [ASYNC_TOKEN, rec](const std::string&) { ASYNC_RELEASE }, url);
}
...
void StremioHome::addFavouritesRow() {
    ...
    if (!this->rowsSubscribed)
        Favourites::instance().changed()->subscribe([this]() { brls::sync([this]() { this->refreshFavourites(); }); });
