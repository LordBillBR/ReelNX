...
void HTTP::configureTLS(void* handle) {
    auto* easy = static_cast<CURL*>(handle);
    curl_easy_setopt(easy, CURLOPT_SSL_VERIFYPEER, 1L);
    curl_easy_setopt(easy, CURLOPT_SSL_VERIFYHOST, 2L);
#ifdef __SWITCH__
    // The Switch has no conventional host CA path. The bundle is shipped in
    // RomFS and contains the public roots needed by Stremio and addons.
    curl_easy_setopt(easy, CURLOPT_CAINFO, "romfs:/certs/ca-bundle.crt");
#else
    const char* caBundle = std::getenv("CURL_CA_BUNDLE");
    if (caBundle && caBundle[0] != '\0') curl_easy_setopt(easy, CURLOPT_CAINFO, caBundle);
#endif
}
...
size_t HTTP::easy_header_cb(char* buffer, size_t size, size_t nitems, void* userdata) {
...
HTTP::Response HTTP::_get_response(const std::string& url) {
    Response response;
    std::ostringstream body;
    curl_easy_setopt(this->easy, CURLOPT_HEADERFUNCTION, easy_header_cb);
    curl_easy_setopt(this->easy, CURLOPT_HEADERDATA, &response);
    curl_easy_setopt(this->easy, CURLOPT_URL, url.c_str());
    curl_easy_setopt(this->easy, CURLOPT_HTTPGET, 1L);
    response.statusCode = this->perform(&body);
