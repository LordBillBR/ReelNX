/*
    Stremio Addon Protocol client for ReelNX
    ...
    the inherited jellyfin::getJSON helper -- minus the auth header, and taking a
...
// Configured via reelnx-addon.txt lines "rpdb=KEY" or "poster=TEMPLATE".
...
// addon. Configured via a "subtitles=URL" line in reelnx-addon.txt.
// Requests look like: {SUBTITLES_ADDON}/subtitles/{type}/{id}.json
inline std::string SUBTITLES_ADDON;
inline std::vector<std::string> SUBTITLES_ADDONS;
...
inline bool waitForRetry(std::chrono::milliseconds delay, const HTTP::Cancel& cancel) {
    const auto interval = std::chrono::milliseconds(100);
    auto waited = std::chrono::milliseconds(0);
    while (waited < delay) {
        if (cancel && cancel->load()) return false;
        auto step = std::min(interval, delay - waited);
...
inline void saveConfig(const std::string& configDir) {
    ...
    try {
        ...
        if (out.is_open())
            out << nlohmann::json{{"url", STREAM_ADDON}, {"urls", STREAM_ADDONS}, {"poster", POSTER_TEMPLATE},
                {"subtitles", SUBTITLES_ADDON}, {"subtitle_urls", SUBTITLES_ADDONS}}
                       .dump(2);
...
inline void loadAddon(const std::string& configDir) {
    try {
        ...
        STREAM_ADDONS.clear();
        for (auto& value : jstrlist(j, "urls")) {
            auto addon = normalizeAddonUrl(value);
            if (!addon.empty()) STREAM_ADDONS.push_back(addon);
        }
        if (STREAM_ADDONS.empty() && !STREAM_ADDON.empty()) STREAM_ADDONS.push_back(STREAM_ADDON);
        if (!STREAM_ADDONS.empty()) STREAM_ADDON = STREAM_ADDONS.front();
        ...
        for (auto& value : jstrlist(j, "subtitle_urls")) {
            auto addon = normalizeAddonUrl(value);
            if (!addon.empty()) SUBTITLES_ADDONS.push_back(addon);
        }
...
inline void importAddonFromFile(const std::string& configDir) {
    const std::string candidates[] = {
        "sdmc:/switch/ReelNX/reelnx-addon.txt",  // next to where the .nro lives
...
inline void getJSON(std::function<void(Result)> then, OnError error, const std::string& url, HTTP::Cancel cancel = nullptr) {
    brls::async([then, error, url, cancel]() {
        ...
        for (int attempt = 0; attempt < maxAttempts; ++attempt) {
            ...
            try {
                auto resp = cancel ? HTTP::getResponse(url, HTTP::Timeout{20000}, cancel)
                                   : HTTP::getResponse(url, HTTP::Timeout{20000});
                statusCode = resp.statusCode;
                retryAfter = resp.retryAfter;
                if (resp.statusCode >= 400) {
                    lastErr = fmt::format("http status {}", resp.statusCode);
                } else if (resp.body.empty()) {
                    lastErr = "empty response";
                } else {
                    auto j = nlohmann::json::parse(resp.body).get<Result>();
            ...
            if (!decision.retry || !waitForRetry(decision.delay, cancel)) break;
        }
        if (cancel && cancel->load()) return;
...
inline std::vector<std::string> extractAudioLangs(const std::string& desc) {
    std::vector<std::string> out;
    static const char* audioMarks[] = {"\xF0\x9F\x8C\x90", "\xF0\x9F\x8C\x8D", "\xF0\x9F\x8C\x8E", "\xF0\x9F\x8C\x8F",
        "\xF0\x9F\x97\xA3", "\xF0\x9F\x94\x8A"};  // 🌐 🌍 🌎 🌏 🗣 🔊
...
inline void fetchStreams(
    const std::string& type, const std::string& id, std::function<void(StreamList, std::string)> then, OnError error) {
    ...
    auto addons = STREAM_ADDONS;
    if (addons.empty() && !STREAM_ADDON.empty()) addons.push_back(STREAM_ADDON);
    if (addons.empty()) {
        error("no stream addon configured");
        return;
    }
    ...
    auto finish = [aggregate, then, error, type]() {
        ...
        if (!aggregate->anyOk && aggregate->streams.empty()) {
            if (error) error(aggregate->lastError.empty() ? "no streams found" : aggregate->lastError);
            return;
        }
        StreamList result;
        result.streams = std::move(aggregate->streams);
        if (then) then(std::move(result), type);
    };
    for (auto& addon : addons)
        for (auto& streamType : *types)
            getJSON<StreamList>(
                [aggregate, finish](StreamList result) {
                    aggregate->anyOk = true;
                    aggregate->streams.insert(aggregate->streams.end(), result.streams.begin(), result.streams.end());
                    finish();
                },
                [aggregate, finish](const std::string& message) {
                    aggregate->lastError = message;
                    finish();
                },
                addon + "/stream/" + streamType + "/" + id + ".json");
}
