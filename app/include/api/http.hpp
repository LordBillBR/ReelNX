...
public:
    ...
    // Applies the platform trust-store policy to a libcurl easy handle.
    static void configureTLS(void* easy);
    ...
    template <typename... Ts>
    static Response getResponse(const std::string& url, Ts&&... ts) {
        HTTP s;
        set_option(s, std::forward<Ts>(ts)...);
        return s._get_response(url);
    }
...
private:
    ...
    static size_t easy_header_cb(char* buffer, size_t size, size_t nitems, void* userdata);
    static int easy_progress_cb(
        void* clientp, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow);
    int perform(std::ostream* body);
    Response _get_response(const std::string& url);
